...
void computeHalfSimpleSystem( double * w0, ParamHalfSimple & lumpPrm )
{
	...
	computeHalfSimpleSystemTD( t, w, lumpPrm, dwdt, auxVars );


	// store initial values
	storeValuesHalfSimpleSystem( i, t, w, auxVars, tStore, wStore, auxStore, lumpPrm.Nvars, lumpPrm.NauxVars );
	...
	while ( i < lumpPrm.Ncyc*lumpPrm.NtStepsCycle )
	{
		...
		computeHalfSimpleTrapezoidIntegrationStep(t, w, lumpPrm, auxVars );
	...
	lumpPrm.printParams();
	// Output result
	outputModelResultHalfSimple( Nsteps, lumpPrm, tStore, wStore, auxStore, w );
...
void computeHalfSimpleTrapezoidIntegrationStep( double tn, double * w, ParamHalfSimple & lumpPrm, double * auxVars )
{
	...
	computeHalfSimpleSystemTD( tn, w, lumpPrm, Fn, auxVars );
	for(int i = 0; i < lumpPrm.Nvars; i++)
	{
    ...
    while ( (vectorNorm(Xdiff,lumpPrm.Nvars,2.0) > lumpPrm.xMinDiff) &&  (vectorNorm(GX,lumpPrm.Nvars,2.0) > lumpPrm.fNewtMin) && (k <=  lumpPrm.maxNewtIter) )
	{
    	...
    	// compute the RHS
		computeHalfSimpleGFunc( tnp1, lumpPrm.dt, w, X, Fn, lumpPrm, auxVars, GX );

		// compute the jacobian
		computeHalfSimpleApproximateJacobian( GX, X, lumpPrm.dX, J,
		...
		double tnp1, double dt, double * wn, double * Fn, ParamHalfSimple & lumpPrm, double * auxVars )
{
    ...
    for ( int k = 0; k < lumpPrm.Nvars; k++ )
	{
        ...
        computeHalfSimpleGFunc( tnp1, dt, wn, XdXk, Fn, lumpPrm, auxVars, GdXk );
...
void computeHalfSimpleGFunc( double tnp1, double dt, double * wn, double * wnp1_approx, double * Fn, ParamHalfSimple & lumpPrm, double * auxVars, double * G )
{
	...
	computeHalfSimpleSystemTD( tnp1, wnp1_approx, lumpPrm, Fnp1_approx, auxVars );
...
void computeHalfSimpleSystemTD( double t, double * w, ParamHalfSimple & lumpPrm, double * dwdt, double * auxVars )
{
	...
	// Ventricular activation 			productActivation( double t, double Tc, double Ta, double Ts, double kt )
	At_ventricle = productActivation( t, lumpPrm.Tc, lumpPrm.Ta_ventricle, lumpPrm.Ts_ventricle, lumpPrm.kt_ventricle);
	// Atrial activation
	At_atrium = productActivation( t, lumpPrm.Tc, lumpPrm.Ta_atrium, lumpPrm.Ts_atrium, lumpPrm.kt_atrium);
